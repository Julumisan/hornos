...
class HornoControl:
    ...
    def __init__(self):
        ...
        self.x, self.y1, self.y2, self.y3 = [], [], [], []  # Listas para graficar los datos
    ...
    def toggle_pause(self): 
        ...
        """
        self.paused = not self.horno_control.paused            
...
class HornoGUI:
    ...
    def setup_gui(self):
        ...
        self.horno_control.max_temp_label = tk.Label(max_temp_frame, text="Temperatura Máxima:")
        ...
        self.horno_control.update_max_temp_button = tk.Button(max_temp_frame, text="Actualizar Máxima", command=self.update_max_temp)
        ...
        self.horno_control.min_temp_label = tk.Label(min_temp_frame, text="Temperatura Mínima:")
        ...
        self.horno_control.update_min_temp_button = tk.Button(min_temp_frame, text="Actualizar Mínima", command=self.update_min_temp)
        ...
        self.horno_control.exitButton = tk.Button(self.horno_control.root, text="SALIR", command=self.horno_control.root.destroy, fg="red")
        ...
        self.horno_control.ani = animation.FuncAnimation(fig, self.grafica_real_time, interval=500)
        self.horno_control.ani_running = self.horno_control.canvas.get_tk_widget().after(0, self.horno_control.ani.event_source.start)
   
    
        # Crea el botón de guardar los datos
        self.horno_control.save_button = tk.Button(self.horno_control.root, text="Guardar datos", command=self.horno_control.save_data)
        self.horno_control.save_button.pack(side=tk.BOTTOM, pady=10)   
        
        # Crea el botón de pausa
        self.horno_control.pause_button = tk.Button(self.horno_control.root, text="Pausar", command=self.horno_control.toggle_pause)
    ...
    def grafica_real_time(self, i : int):
        ...
        if not self.horno_control.paused:
            ...
            self.horno_control.tempG
            self.horno_control.max_temp
            self.horno_control.min_temp
            ...
            # self.horno_control.tempG = self.horno_control.random_number_between_20_and_40(self.horno_control.tempG)
            ...
            print("Tiempo: {:.2f}s, Temperatura {:.2f}".format(self.horno_control.count*0.5,self.horno_control.tempG))
            ...
            if self.horno_control.max_temp < self.horno_control.min_temp:
                self.horno_control.max_temp = self.horno_control.min_temp + 2
            
            if self.horno_control.max_temp < self.horno_control.tempG:
                print("Estamos calientes")
                # self.horno_control.tempG = self.horno_control.random_number_between_20_and_40(self.horno_control.tempG, -0.2)
    ...
    def update_max_temp(self):
        ...
        # Si se mete una temperatura, la convierte en un número flotante y la actualiza.
        if temp_str:
            self.horno_control.max_temp = float(temp_str)
    ...
    def update_min_temp(self):
        ...
        if temp_str:
            self.horno_control.min_temp = float(temp_str)
